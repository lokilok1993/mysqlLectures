...
/* Аналогичный запрос с внешним соединением, но с таблицей individual вместо business */
...
/* Левостороннее соединение включает в результатирующий набор все значения из левой таблицы в блоке FROM
   и если ему соответсвуют значения из правой добавляет их. Правосторонее соединение работает обратным образом */
...
/* ( Соединение с тремя таблицами ) список всех счетов с указанием или имени и фамилии физического лица, или названия фирмы для юридического лица: */
...
/* Чтобы не городить много соединений можно воспользоваться подзапросами */
  ...
  FROM (
    ...
    FROM account a LEFT OUTER JOIN individual i
    ON a.cust_id = i.cust_id
  ) account_ind LEFT OUTER JOIN business b
  ON account_ind.cust_id = b.cust_id;



/* ( Внешнее соединение таблицы с самой собой ) Выбрать всех сотрудников и их начальников если они есть */
...
/* Натуральные соединения по идентиным именам столбцов */
...
/* Напишите запрос, возвращающий все типы счетов и открытые счета этих типов
  ...
  SELECT p.product_cd, a.cust_id, a.account_id, a.avail_balance
...
/* Переформулируйте запрос из упражнения 10.1 и примените другой
   тип внешнего соединения (т. е. если в упражнении 10.1 использовалось левостороннее внешнее соединение,
   используйте правостороннее), так чтобы результаты были, как в упражнении 10.1. */
...
/* Проведите внешнее соединение таблицы account с таблицами individual и business (посредством столбца account.cust_id)
   таким образом, чтобы результирующий набор содержал по одной строке для каждого счета. Должны быть включены столбцы
   count.account_id, account.product_cd, individual.fname, individual.lname и business.name. */
...
/* Выбрать или имя клиента физ лица или имя клиента компании через условную логику */
...
/* Выражение case с перебором вариантов ( search case expression ) [ синтаксис ] */
...
/* Выражения case могу возвращать значения любого типа, в том числе подзапросы */
...
/* Простые выражения case где сравнение идёт с указанным в ночале значением [ синтаксис ] */
...
-- Транзакции и индексы не нуждаются в выполнении примеров
